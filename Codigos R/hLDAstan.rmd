...
```{r}
#load("HLDAStanCorrido.RData")
```

```{r}
Datos <- fread("MatrixFinal6m46cli.csv")
```

```{r}
colnames(Datos)[1] = "household_key"
```


```{r}
Datos <- merge(Datos, Data, by = "household_key")
```

```{r}
length(unique(Datos$Boleta))
```


```{r}
#se guarda solo la boleta y los productos que se compraron en cada boleta
trans <- ddply(Datos,c("Boleta"),
...
```{r}
document <- c(1:length(unique(trans$Boleta)))
trans <- cbind(trans, document)
```


```{r}
transDoc <- trans
transDoc <- merge(trans, Datos, by = "Boleta")
transDoc <- transDoc %>% group_by(Boleta) %>% slice(1)
transDoc <- transDoc[,2:84]
```


```{r}
trans <- select(trans, V1)
...
```{r}
set.seed(1)
trans <- sample_n(trans, size = 20)

```

```{r}
set.seed(1)
transDoc <- sample_n(transDoc, size = 20)

transDoc$document <- c(1:20)
...
write.csv(trans,"PruebaHLDA.csv", quote = FALSE, row.names = FALSE)
```
...
tr <- read.transactions('PruebaHLDA.csv', format = 'basket', sep=',', header = TRUE)
```
...
```{r}
set.seed(1)
#t_sample <- t[sample(nrow(t), 100),]

dtm <- as.DocumentTermMatrix(t, weighting = weightTf)

#ind <- row_sums(dtm) == 0 #quitar documentos sin items
#dtm <- dtm[!ind,]
...
```{r}
ap_t1 <- merge(ap_t1, transDoc, by = "document")
ap_t1 <- select(ap_t1, document, term, Recency)
...
```{r}
#vector <- transDoc$Recency
#vectoraux <- transDoc$Recency

#for (i in 1:15) {
#  vector <- cbind(vector,vectoraux)
#}
...
model2_stan <- "
data {
  int<lower=2> K;               // num motivations
  int<lower=2> V;               // num product categories
  int<lower=1> M;               // num transactions
  int<lower=1> N;               // total product instances
  int<lower=1,upper=V> w[N];    // product n
  int<lower=1,upper=M> doc[N];  // trx ID for prod n
  vector<lower=0>[V] beta;      // Product prior
  int<lower = 1> D;  // num of covariables to predict Alpha
  matrix[M, D] x; //covariables data for each trx M

}
parameters {

  //vector<lower=0>[K] alpha;     // Alpha for each trx
  matrix[D, K] delta;   //importance of variable D in mot K
...
vector<lower=0>[K] alpha[M];     // Alpha for each trx
matrix[M, K] x_delta = x * delta;
for (m in 1:M)
    //  for (k in 1:K)
    //    alpha[m,k] = lambda*inv_logit(x_delta[m,k]);;
    alpha[m] = lambda*softmax(x_delta[m]');
}


model {
 lambda  ~ uniform(0.1,15);
for (k in 1:K)
   delta[,k] ~ normal(0,5);
//for (k in 1:K)
//  alpha[k] ~ uniform(0,2); 
  ...
  for (m in 1:M)
    theta[m] ~ dirichlet(alpha[m]);  // prior
  for (n in 1:N) {
    real gamma[K];
    for (k in 1:K)
...
```{r}
prueba <- select(ap_t1, document, term, Recency)
prueba
```
...
```{r}
Vector1 <- transDoc$Recency
Vector2 <- transDoc$NuTrx
Vectores <- cbind(Vector1, Vector2)
```
...
```{r}
thetaStan=matrix(data=summary(model1_fit, pars ="theta")$summary[,1],20,2, byrow= TRUE)
...
```{r}
save.image(file = "HLDAStanCorrido.RData")
```
